...
try {
  ...
  console.warn("⚠️ connect-mongo is not installed; falling back to default session storage");
}
...
// Only enable when running in production or when explicitly requested via
// the TRUST_PROXY environment variable to avoid permissive proxy warnings
...
const bootstrap = async () => {
  ...
  if (dbConnected) {
    ...
    console.warn("⚠️ Skipping admin seed because MongoDB is unavailable.");
  }
...
app.use((err, req, res, next) => {
  ...
  if (req.accepts("html")) {
    ...
    const view = views[statusCode] || "500";
