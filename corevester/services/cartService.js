...
// When a customer adds quantity to the cart, Product.units is
// reduced immediately. The reduction is atomic with the cart
// update, so two customers cannot reserve the same units.
// Checkout does NOT reduce Product.units a second time.
// =========================================================
  ...
  try {
    ...
    await dbSession.withTransaction(async () => {
      const product = await Product.findOneAndUpdate(
        { _id: productId, isActive: true, units: { $gte: qty } },
        { $inc: { units: -qty } },
