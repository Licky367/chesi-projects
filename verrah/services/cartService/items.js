...
async function addToCart(req, productId, quantity) {
    ...
    if (product.units !== undefined && Number(product.units) < qty) {
        throw new Error(`Only ${product.units} unit(s) available.`);
    }
    ...
    if (existingItem) {
        const newQty = Number(existingItem.qty || 0) + qty;
        if (product.units !== undefined && Number(product.units) < newQty) {
            throw new Error(`Only ${product.units} unit(s) available.`);
        }
        existingItem.qty = newQty;
        if (existingItem.price === undefined || existingItem.price === null) {
            existingItem.price = Number(product.unitSellPrice || 0);
        }
    } else {
        cart.items.push({
            product: product._id,
            ...
            name: product.name || "",
            price: Number(product.unitSellPrice || 0),
            image: product.image || "",
            qty
        });
    }
...
module.exports = { addToCart, removeItem };
